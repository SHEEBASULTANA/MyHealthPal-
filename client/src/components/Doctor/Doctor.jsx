...
function Doctor() {
  ...
  function SignUp() {
    if (cpass === passd) {
      ...
      axios.post('http://localhost:4000/doctor/signup', postData)
        .then(response => {
  ...
  function SignIn() {
    ...
    axios.post('http://localhost:4000/doctor/signin', postData)
      .then(response => {
        if(response.data.data!=="no"){
          navigate('/doctor/login',{ replace: true , state: { data: response.data.data } });
        }
      })
      .catch(error => {
        console.error('Error sending data:', error);
